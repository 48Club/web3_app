...
const Miner: React.FC<{
    ...
    return <Tooltip overlayClassName="miner-address-detail-copy" trigger={["hover", "click"]} title={<Typography.Paragraph className="icon-white" copyable>{miner.address}</Typography.Paragraph>} color="#000" key={miner.address}>
        <span onClick={(e) => e.stopPropagation()} key={miner.address} className={`bg-[rgba(255,200,1,.1)] cursor-pointer mr-[12px] px-[10px] py-[2px] rounded-full flex items-center ${className ?? ''}`}>
